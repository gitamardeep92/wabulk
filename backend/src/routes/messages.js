...
router.post('/send', requireAuthOrApiKey, checkPlanLimits, [
  ...
  // Count actual sent messages this month from messages table
...
router.get('/campaigns/:id', requireAuth, async (req, res) => {
  ...
  const { data: messages } = await supabase
    .from('messages')
    .select('id, to_number, rendered_body, status, sent_at, failed_at, error_message')
    .eq('campaign_id', campaign.id).order('queued_at', { ascending: true });

  // Auto-fix: any message stuck in 'sending' for a cancelled/completed campaign → failed
  const stuck = (messages || []).filter(m =>
    m.status === 'sending' && ['cancelled', 'completed'].includes(campaign.status)
  );
  if (stuck.length) {
    await supabase.from('messages')
      .update({ status: 'failed', error_message: 'Campaign ended while sending' })
      .in('id', stuck.map(m => m.id));
    stuck.forEach(m => { m.status = 'failed'; m.error_message = 'Campaign ended while sending'; });
  }
...
router.delete('/campaigns/:id', requireAuth, async (req, res) => {
  ...
  // Mark queued AND stuck sending messages as failed
  await supabase.from('messages')
    .update({ status: 'failed', error_message: 'Cancelled by user', failed_at: new Date().toISOString() })
...
router.get('/stats', requireAuth, async (req, res) => {
  ...
  const [limitsRes, campaignsRes, monthSentRes, sentRes, failedRes] = await Promise.all([
    ...
    // Monthly usage = messages that reached 'sent' this month
    supabase.from('messages').select('*', { count: 'exact', head: true })
      .eq('user_id', userId).eq('status', 'sent').gte('queued_at', monthStart),
    // Sent last 30 days
    supabase.from('messages').select('*', { count: 'exact', head: true })
      .eq('user_id', userId).eq('status', 'sent').gte('queued_at', thirtyDaysAgo),
    // Failed last 30 days
    supabase.from('messages').select('*', { count: 'exact', head: true })
